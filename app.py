...
def predict():
    with open('pkl_folder/LR_model.pkl', 'rb') as file:
        classifier = joblib.load(file)

    with open('pkl_folder/tfidf_vectorizer.pkl', 'rb') as file:
        tfidf_vectorizer = joblib.load(file)

    if request.method == 'POST':
        message = request.form['message']
        message = clean_sentence(message)
        new_sentence_features = tfidf_vectorizer.transform([message])
        prediction = classifier.predict_proba(new_sentence_features)[:, 1]
